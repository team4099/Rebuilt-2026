...
interface ClimbIO {
    class ClimbInputs : LoggableInputs {
        ...
        override fun fromLog(table: LogTable?) {
            table?.get("climbPositionInches", climbPosition.inInches)?.let {
                climbPosition = it.inches
            }
            table?.get("climbAppliedVoltage", appliedVoltage.inVolts)?.let {
                appliedVoltage = it.volts
            }
            table?.get("climbTempCelsius", temperature.inCelsius)?.let {
                temperature = it.celsius
            }
            table?.get("climbSupplyCurrent", supplyCurrent.inAmperes)?.let {
                supplyCurrent = it.amps
            }
            table?.get("climbStatorCurrent", statorCurrent.inAmperes)?.let {
                statorCurrent = it.amps
            }
        }
    }
